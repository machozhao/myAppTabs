...
.factory('Messages',['SERVER','$interval', '$log','$http','$q',function(SERVER,$interval,$log,$http,$q) {
  return {
	 	getChatMsgs:function(chatSessionId, startSendTime, endSendTime ,pageNo, pageSize){

      var url = SERVER.SERVER_ADDR+SERVER.URL_Message_GET;
      var deferred = $q.defer(); // 声明延后执行，表示要去监控后面的执行
      $http.get(url,{cache: false,
          params:{'chatSessionId':chatSessionId,'fromId':'','toId':'','startSendTime':startSendTime
                  ,'endSendTime':endSendTime,'msgContent':'','pageNo':pageNo,'pageSize':pageSize}})
      .success(function(data, status, headers, config) {
           deferred.resolve(data);  // 声明执行成功，即http请求数据成功，可以返回数据了
      }) .error(function(data, status, headers, config) {
          deferred.reject(data);   // 声明执行失败，即服务器返回错误
